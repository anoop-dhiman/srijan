...
|   +-- srijan_workspaces        # Git repos / project files
|   +-- srijan_data              # Platform state (SQLite)
|   +-- caddy_data               # TLS certs, OCSP staples
...
- **Volumes**: named Docker volumes (`srijan_workspaces`, `srijan_data`, `caddy_data`, `caddy_config`) — no host paths needed
