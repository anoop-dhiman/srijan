...
```
VM (Ubuntu 22.04+)
+-- ~/srijan/  (or chosen install dir)
|   +-- docker-compose.yml       # Written by setup.sh (heredoc)
|   +-- .env                     # Secrets + config (chmod 600, generated by setup.sh)
|   +-- workspaces/              # Git repos / project files (bind-mounted into platform)
|   +-- data/                    # Platform state: SQLite DB (bind-mounted into platform)
|   +-- caddy/
|       +-- Caddyfile            # Written by setup.sh (caddy or external-tls variant)
...
The script is **fully interactive** (reads prompts via `/dev/tty` so it works with `curl | bash`):

1. Checks dependencies: `docker`, `docker compose`, `openssl` — fails fast with install instructions if missing
2. Prompts: domain, TLS mode (`caddy` = auto HTTPS / `external` = upstream LB handles TLS), email (Let's Encrypt, caddy mode only), admin password (hidden + confirm, min 8 chars), install directory (default `$(pwd)/srijan`)
3. Writes `docker-compose.yml` and the appropriate `Caddyfile` as heredocs into the install dir — no git clone needed
4. Pulls `ghcr.io/anoop-dhiman/srijan-platform:latest`
5. Generates `JWT_SECRET` and `SECRETS_KEY` via `openssl rand`; writes `.env` (mode 600)
6. Re-runs preserve existing secrets and only update domain/email/password
7. `docker compose up -d`, waits for the health check, prints the access URL

No `sudo` required. Trap on INT/TERM for a clean exit message.

### Docker Compose (Platform)

The `setup.sh` script writes `docker-compose.yml` into the install directory as a heredoc. Key points:

- **Image**: `ghcr.io/anoop-dhiman/srijan-platform:latest` (prebuilt, pushed by CI)
- **Volumes**: relative bind mounts (`./workspaces`, `./data`, `./caddy/data`, `./caddy/config`) — data lives alongside `docker-compose.yml` in the install dir, easy to inspect and back up
- **Secrets**: passed via `.env` file (`SRIJAN_ADMIN_PASSWORD`, `SRIJAN_JWT_SECRET`, `SRIJAN_SECRETS_KEY`) using `:?` syntax so compose fails fast if missing
- **Resource limits**: Caddy: 0.5 CPU / 128 MB; Platform: 2 CPU / 1 GB
- **`version:` key removed** — not needed with modern Docker Compose
...
services:
  ...
  platform:
    ...
    volumes:
      ...
      - srijan_data:/data
    environment:
      - SRIJAN_ADMIN_PASSWORD=${SRIJAN_ADMIN_PASSWORD:?}
      - SRIJAN_JWT_SECRET=${SRIJAN_JWT_SECRET:?}
      - SRIJAN_SECRETS_KEY=${SRIJAN_SECRETS_KEY:?}
...
|   |   +-- __tests__/           # 279 backend tests (vitest, forks pool)
...
|           +-- __tests__/        # 196 frontend unit tests (vitest)
|           +-- e2e/              # 22 Playwright E2E tests (5 spec files)
+-- deployment/
|   +-- setup.sh                 # Interactive setup: prompts, heredoc compose+Caddyfile, pulls prebuilt image
|   +-- docker-compose.yml       # Caddy + Platform compose (named volumes, resource limits)
|   +-- docker-compose.dev.yml   # Dev overrides (hot reload, port 2019 exposed)
|   +-- caddy/
|       +-- Caddyfile            # Base routing config (caddy TLS mode)
|       +-- Caddyfile.external-tls # Reference: external TLS (auto_https off, trusted_proxies)
```
